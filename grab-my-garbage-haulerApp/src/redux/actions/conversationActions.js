...
export const getConversations = (id, token) => async(dispatch) => {
    try{
        dispatch({
        ...
        const { data } = await axios.get(`https://grab-my-garbage-server.herokuapp.com/conversation/${id}`, config)
        //const { data } = await axios.get(`http://192.168.13.1:5000/conversation/${id}`, config)
